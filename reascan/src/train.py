...
def train_one_epoch(model, train_dataloader, command_lang, action_lang, 
    ...
    for batch_index, data in tqdm(enumerate(train_dataloader)):
        ...
        if config.num_of_streams == 'dual':
            ...
            action_pred, _, _, _, _ = model(batch_text, batch_world, batch_world_loc, 
                                            batch_target_input, batch_text_mask, batch_world_mask, 
                                            batch_target_mask_input,)
...
def validate_one_epoch(model, val_dataloader, command_lang, 
    ...
    with torch.no_grad():
        for batch_index, data in tqdm(enumerate(val_dataloader)):
            ...
            if not exact_match:
                ...
                if config.num_of_streams == 'dual':
                    decoded_batch, _ = greedy_decode_batch_dual(model, batch_text, batch_text_mask, 
                                                                batch_world, batch_world_loc, batch_world_mask, 
                                                                config, device, max_len=config.target_max_position_embeddings)
                elif config.num_of_streams == 'single':
                    decoded_batch = greedy_decode_batch_single(model, batch_text, batch_text_mask, 
                                                                  batch_world, batch_world_loc, batch_world_mask, 
                                                                  config, device, max_len=config.target_max_position_embeddings)
...
def greedy_decode_dual(model, batch_text, batch_text_mask, batch_world, 
...
def greedy_decode_batch_dual(model, batch_text, batch_text_mask, batch_world, 
                             batch_world_loc, batch_world_mask, config, 
...
def greedy_decode_batch_single(model, batch_text, batch_text_mask, batch_world, 
                             batch_world_loc, batch_world_mask, config, 
                             device, max_len=105, start_symbol=1):
    """Greedy decode batch for testing
    """
